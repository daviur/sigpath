...
MINIDUMP_TYPE = FlagsEnum(ULInt64("Flags"),
...
# Context for i386 arch (See winnt.h)
...
MINIDUMP_FUNCTION_TABLE_DESCRIPTOR = Struct(
    ...
    HexDumpAdapter(String('NativeDescriptor',
                          lambda ctx: ctx._.SizeOfDescriptor)),
    Array(lambda ctx: ctx.EntryCount,
          HexDumpAdapter(String('FunctionEntries',
...
MINIDUMP_MEMORY_LIST = Struct('MINIDUMP_MEMORY_LIST',
                              ...
                              Array(lambda ctx: ctx.NumberOfMemoryRanges,
...
MINIDUMP_HANDLE_DESCRIPTOR_2 = Struct('MINIDUMP_HANDLE_DESCRIPTOR_2',
                                      ...
                                      NullPointer(lambda ctx: ctx.ObjectInfoRva,
                                                  MINIDUMP_HANDLE_OBJECT_INFORMATION),
                                      ULInt32('Reserved0'),
)
...
MINIDUMP_DIRECTORY = Struct('MINIDUMP_DIRECTORY',
                            ...
                            Switch("DirectoryData", lambda ctx: ctx.StreamType, {
                                ...
                                'CommentStreamW': DirectoryEntry(
                                    String("MINIDUMP_COMMENT_W", lambda ctx: ctx.Location.DataSize,
                                           encoding='utf-16-le')
                                ),
                            },
...
def main(argv=None):
    minidumpfile = _process_cmd_line(argv)
    print(MINIDUMP_HEADER.parse_stream(open(minidumpfile)))
    return 0
